...
Output the manufacturer's name, the number of drugs associated with losses, and the total
...
/** A query to identify the manufacturers associated with the drugs that resulted in
...
and the total losses in absolute value. Display the results sorted in descending 
order with the highest losses displayed at the top.**/

SELECT manufacturer,
CONCAT('$', ROUND(SUM(total_sales) / 1000000,4),'million' ) 
    As Sale FROM pharmacy_sales
    GROUP BY  manufacturer
